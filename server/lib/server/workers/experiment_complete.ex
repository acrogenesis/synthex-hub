defmodule Server.Workers.ExperimentComplete do
  @moduledoc """
  Final job in an experiment's lifecycle. Runs once when
  `ExperimentCegarIter` has exhausted `cegar_rounds × max_iters`.

  Responsibilities:

    1. Run the final validation pass against `Synthex.Gym.Mujoco.validation_seeds/0`.
    2. Persist the final reward as `best_reward` (we keep the
       running max throughout the run; here we just confirm).
    3. Flip `status → completed` and stamp `completed_at`.
    4. Emit a `"info"` `system_event` noting the run finished.

  Idempotent: if it runs twice it just re-validates and re-stamps;
  no duplicate side effects.
  """

  use Oban.Worker, queue: :master, max_attempts: 3
  require Logger

  alias Server.{Experiments, Experiment}
  alias Server.Workers.ExperimentBootstrap
  alias Synthex.Core.PrettyPrint

  @impl Oban.Worker
  def perform(%Oban.Job{args: %{"experiment_id" => id}}) do
    case Experiments.get(id) do
      {:error, :not_found} ->
        {:discard, "experiment not found"}

      {:ok, %Experiment{status: status}} when status in ["completed", "failed", "cancelled"] ->
        :ok

      {:ok, %Experiment{} = exp} ->
        finalize(exp)
    end
  end

  defp finalize(%Experiment{} = exp) do
    env_key = ExperimentBootstrap.decode_env_key(exp.env_key)
    ctx = ExperimentBootstrap.build_context(env_key, exp.config, exp.id)

    preds =
      case exp.predicates do
        %{"preds" => list} when is_list(list) -> Enum.map(list, &PrettyPrint.from_json_term/1)
        _ -> []
      end

    val_seeds = Synthex.Gym.Mujoco.validation_seeds()
    {total_reward, survived} = Synthex.Gym.Mujoco.validate(preds, val_seeds, ctx)
    final_avg = total_reward / length(val_seeds)

    Logger.info(
      "[Complete] #{exp.env_name} final validation: avg=#{Float.round(final_avg, 2)}/ep " <>
        "survived=#{survived}/#{length(val_seeds)}"
    )

    {:ok, _exp} =
      Experiments.update_state(exp, %{
        "status" => "completed",
        "completed_at" => DateTime.utc_now(),
        "best_reward" => final_avg
      })

    Experiments.log_event!(
      "info",
      "master",
      "experiment completed: #{exp.env_name} final_avg=#{Float.round(final_avg, 2)} " <>
        "(baseline #{Float.round(exp.baseline_reward || 0.0, 2)}, " <>
        "#{exp.accepted_count} bits accepted)",
      env_name: exp.env_name,
      experiment_id: exp.id,
      metadata: %{
        "final_avg" => final_avg,
        "survived" => survived,
        "baseline_reward" => exp.baseline_reward,
        "accepted_count" => exp.accepted_count
      }
    )

    :ok
  end
end
