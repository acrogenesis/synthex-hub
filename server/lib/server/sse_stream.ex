defmodule Server.SSEStream do
  @moduledoc """
  Minimal Server-Sent Events handler. Parameterized over the data
  source so multiple feeds (load metrics, per-batch aggregates, …)
  can share the same wire-format / lifecycle code.
  ...
  Bandit processes from accumulating during traffic spikes.
  ...
  Notes:
    ...
    * `Process.sleep/1` blocks ONE Bandit process per connection; in
      ...
      cost ~MBs at most.
    * We push from a `latest_fn` that should hit an in-process ETS
      cache (never the DB) — per-client cost is constant regardless
      of viewer count.
  ...
  ## Usage
      ...
      Server.SSEStream.serve(conn,
        latest_fn: &Server.MetricsBroker.latest/0,
  ...
  @default_event_interval_ms 1_000
  @default_max_duration_ms 5 * 60 * 1_000

  @type opts :: [
          latest_fn: (-> term()),
          event_interval_ms: pos_integer(),
          max_duration_ms: pos_integer()
        ]

  @spec serve(Plug.Conn.t(), opts) :: Plug.Conn.t()
  def serve(conn, opts) when is_list(opts) do
    latest_fn = Keyword.fetch!(opts, :latest_fn)
    interval_ms = Keyword.get(opts, :event_interval_ms, @default_event_interval_ms)
    ...
    stream_loop(conn, started, latest_fn, interval_ms, max_duration_ms)
  end

  defp stream_loop(conn, started_at, latest_fn, interval_ms, max_duration_ms) do
    payload = encode_event(latest_fn.())
    ...
    case chunk(conn, payload) do
      {:ok, conn} ->
        if System.monotonic_time(:millisecond) - started_at > max_duration_ms do
          ...
          stream_loop(conn, started_at, latest_fn, interval_ms, max_duration_ms)
        end
